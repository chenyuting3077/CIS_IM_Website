...
from .transformer import FocalNet
import torch
import torch.nn as nn
import einops
@MODELS.register_module()
class FocalPixelDecoder(BaseModule):
    ...
    def __init__(self,
        ...
        self.conv_list = [
            nn.Conv2d(96 * (2 ** i), 256, kernel_size=1, stride=1, padding='same', bias=False).to('cuda:0')
            for i in range(4)
        ]
    ...
    def forward(self, feats: List[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        # iterate over the torch list and apply conv layer
        # outputs = []
        # for i in range(4):
        #     outputs.append(self.conv_list[i](feats[i]))
        # self.encoder(query=encoder_inputs)
        # mask_feature = outputs[0]
        # multi_scale_features = [i for i in outputs[1:]]
