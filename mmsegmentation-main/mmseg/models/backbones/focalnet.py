...
class FocalNet(nn.Module):
    ...
    Args:
        pretrain_img_size (int): Input image size for train the pretrained model,
    ...
    def init_weights(self, pretrained="data/pretrained/focalnet_tiny_lrf.pth"):
    ...
    def forward(self, x):
        """Forward function."""
        tic = time.time()
        # turn list of tensors into a tensor
        # x = torch.stack(x, dim=0)  # B C H W
        # to float tensor

        ## using mask focal modualtion
    ...
    def train(self, mode=True):
        """Convert the model into train mode while keep layers freezed."""
